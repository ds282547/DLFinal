...
from MidiGenRBM.midi.utils import midiread, midiwrite
...
def build_rbm(v, W, bv, bh, k):
    ...
    def xlogy(x, y):
        return torch.mul(x,torch.log(y))
    ...
    monitor = xlogy(v) + xlogy(1 - v, 1 - mean_v)
    monitor = monitor.sum() / v.shape[0]

    def free_energy(v):
        return -(v * bv).sum() - torch.log(1 + torch.exp(torch.dot(v, W) + bh)).sum()
...
def shared_normal(num_rows, num_cols, scale=1):
    ...
    elements.'''
    v = Variable(torch.zeros((num_rows, num_cols)))
    v.normal_(mean=0, std=scale)
    return v


def shared_zeros(*shape):
    '''Initialize a vector shared variable with zero elements.'''
    v = Variable(torch.zeros(shape))
    return v
...
def build_rnnrbm(n_visible, n_hidden, n_hidden_recurrent):
    ...
    u0 = torch.zeros((n_hidden_recurrent,))  # initial value for the RNN hidden
                                         # units
    ...
    def recurrence(v_t, u_tm1):
        bv_t = bv + torch.dot(u_tm1, Wuv)
        bh_t = bh + torch.dot(u_tm1, Wuh)
        generate = v_t is None
        if generate:
            v_t, _, _, updates = build_rbm(torch.zeros((n_visible,)), W, bv_t,
                                           bh_t, k=25)
        u_t = torch.tanh(bu + torch.dot(v_t, Wvu) + torch.dot(u_tm1, Wuu))
